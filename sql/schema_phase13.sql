...
grant execute on function public.atlas_financial_summary() to authenticated;

-- RLS de contabilidad por empresa.
drop policy if exists chart_accounts_company_all on chart_of_accounts;
create policy chart_accounts_company_all on chart_of_accounts
for all using(company_id=public.current_company_id())
with check(company_id=public.current_company_id());

drop policy if exists journal_entries_company_all on journal_entries;
create policy journal_entries_company_all on journal_entries
for all using(company_id=public.current_company_id())
with check(company_id=public.current_company_id());

drop policy if exists journal_lines_company_select on journal_lines;
create policy journal_lines_company_select on journal_lines
for select using(
 exists(select 1 from journal_entries e
        where e.id=journal_entry_id and e.company_id=public.current_company_id())
);

-- Nota de diseño:
-- En la nube, cada operación crítica debe contabilizarse en la misma transacción PostgreSQL
-- que modifica inventario/caja/CxC/CxP. La Fase 11 deja vistas, cuentas y resumen preparados
-- para esa integración transaccional completa.
\n\n-- ===== FASE 12: CONTABILIDAD TRANSACCIONAL EN LA NUBE =====\n\ncreate or replace function public.atlas_account_id(p_name text)\nreturns uuid language sql stable security definer set search_path=public as $$\n select id from chart_of_accounts where company_id=public.current_company_id() and name=p_name and coalesce(active,true)=true limit 1\n$$;\ngrant execute on function public.atlas_account_id(text) to authenticated;\n\ncreate or replace function public.atlas_post_journal(p_reference text,p_description text,p_lines jsonb)\nreturns uuid language plpgsql security definer set search_path=public as $$\ndeclare v_company uuid:=public.current_company_id(); v_entry uuid:=gen_random_uuid(); v_line jsonb; v_debit numeric(18,2):=0; v_credit numeric(18,2):=0;\nbegin\n if v_company is null then raise exception 'Empresa no disponible'; end if;\n for v_line in select * from jsonb_array_elements(p_lines) loop\n  v_debit:=v_debit+coalesce((v_line->>'debit')::numeric,0); v_credit:=v_credit+coalesce((v_line->>'credit')::numeric,0);\n end loop;\n if round(v_debit,2)<>round(v_credit,2) then raise exception 'Asiento descuadrado. Debe %, Haber %',v_debit,v_credit; end if;\n insert into journal_entries(id,company_id,entry_date,reference,description,status,created_by) values(v_entry,v_company,now(),p_reference,p_description,'Contabilizado',auth.uid());\n for v_line in select * from jsonb_array_elements(p_lines) loop\n  insert into journal_lines(journal_entry_id,account_id,debit,credit) values(v_entry,(v_line->>'account_id')::uuid,coalesce((v_line->>'debit')::numeric,0),coalesce((v_line->>'credit')::numeric,0));\n end loop;\n return v_entry;\nend $$;\ngrant execute on function public.atlas_post_journal(text,text,jsonb) to authenticated;\n\ncreate or replace function public.atlas_create_sale(p_branch_id uuid,p_customer_id uuid,p_payment_condition text,p_cash_account_id uuid,p_items jsonb)\nreturns jsonb language plpgsql security definer set search_path=public as $$\ndeclare v_company uuid:=public.current_company_id(); v_sale_id uuid:=gen_random_uuid(); v_number text; v_subtotal numeric(18,2):=0; v_tax numeric(18,2):=0; v_total numeric(18,2):=0; v_cogs numeric(18,2):=0; v_item jsonb; v_product products%rowtype; v_qty numeric(14,3); v_stock numeric(14,3); v_cash uuid:=public.atlas_account_id('Caja y bancos'); v_ar uuid:=public.atlas_account_id('Cuentas por cobrar'); v_sales uuid:=public.atlas_account_id('Ventas'); v_vat uuid:=public.atlas_account_id('IVA por pagar'); v_cogs_acc uuid:=public.atlas_account_id('Costo de ventas'); v_inv uuid:=public.atlas_account_id('Inventario');\nbegin\n perform public.atlas_guard_permission('sales'); if not public.can_access_branch(p_branch_id) then raise exception 'Sucursal no autorizada'; end if; if jsonb_array_length(p_items)=0 then raise exception 'La venta no tiene productos'; end if;\n select 'V-'||lpad((count(*)+1)::text,5,'0') into v_number from sales where company_id=v_company;\n for v_item in select * from jsonb_array_elements(p_items) loop\n  select * into v_product from products where id=(v_item->>'product_id')::uuid and company_id=v_company; v_qty:=(v_item->>'qty')::numeric; if v_product.id is null or v_qty<=0 then raise exception 'Producto o cantidad inválida'; end if;\n  select stock into v_stock from inventory where company_id=v_company and branch_id=p_branch_id and product_id=v_product.id for update; if coalesce(v_stock,0)<v_qty then raise exception 'Stock insuficiente para %',v_product.name; end if;\n  v_subtotal:=v_subtotal+v_qty*v_product.price; v_tax:=v_tax+v_qty*v_product.price*v_product.tax/100; v_cogs:=v_cogs+v_qty*v_product.cost;\n end loop; v_total:=v_subtotal+v_tax;\n insert into sales(id,company_id,branch_id,customer_id,document_number,subtotal,tax,total,payment_condition,status) values(v_sale_id,v_company,p_branch_id,p_customer_id,v_number,v_subtotal,v_tax,v_total,p_payment_condition,'Completada');\n for v_item in select * from jsonb_array_elements(p_items) loop\n  select * into v_product from products where id=(v_item->>'product_id')::uuid; v_qty:=(v_item->>'qty')::numeric;\n  insert into sale_items(sale_id,product_id,quantity,unit_price,tax_rate) values(v_sale_id,v_product.id,v_qty,v_product.price,v_product.tax);\n  update inventory set stock=stock-v_qty where company_id=v_company and branch_id=p_branch_id and product_id=v_product.id;\n  insert into inventory_movements(company_id,branch_id,product_id,movement_type,quantity,note,created_by) values(v_company,p_branch_id,v_product.id,'VENTA',-v_qty,v_number,auth.uid());\n end loop;\n if p_payment_condition='cash' then\n  if p_cash_account_id is null then raise exception 'Cuenta de caja requerida'; end if; update cash_accounts set balance=balance+v_total where id=p_cash_account_id and company_id=v_company; insert into cash_movements(company_id,cash_account_id,movement_type,amount,reference,note) values(v_company,p_cash_account_id,'IN',v_total,v_number,'Venta de contado');\n else insert into receivables(company_id,customer_id,reference,issue_date,due_date,total,balance,status) values(v_company,p_customer_id,v_number,current_date,current_date+30,v_total,v_total,'Pendiente'); end if;\n perform public.atlas_post_journal(v_number,'Venta',jsonb_build_array(\n  jsonb_build_object('account_id',case when p_payment_condition='cash' then v_cash else v_ar end,'debit',v_total,'credit',0),\n  jsonb_build_object('account_id',v_sales,'debit',0,'credit',v_subtotal),\n  jsonb_build_object('account_id',v_vat,'debit',0,'credit',v_tax),\n  jsonb_build_object('account_id',v_cogs_acc,'debit',v_cogs,'credit',0),\n  jsonb_build_object('account_id',v_inv,'debit',0,'credit',v_cogs)));\n perform public.atlas_write_audit('CREATE','sale',jsonb_build_object('sale_id',v_sale_id,'number',v_number,'total',v_total,'cogs',v_cogs));\n return jsonb_build_object('id',v_sale_id,'number',v_number,'subtotal',v_subtotal,'tax',v_tax,'total',v_total,'cogs',v_cogs);\nend $$;\n\ncreate or replace function public.atlas_create_purchase(p_branch_id uuid,p_supplier_id uuid,p_payment_condition text,p_cash_account_id uuid,p_product_id uuid,p_qty numeric,p_cost numeric)\nreturns jsonb language plpgsql security definer set search_path=public as $$\ndeclare v_company uuid:=public.current_company_id(); v_purchase_id uuid:=gen_random_uuid(); v_number text; v_tax_rate numeric:=0; v_subtotal numeric(18,2); v_tax numeric(18,2); v_total numeric(18,2); v_inv uuid:=public.atlas_account_id('Inventario'); v_vat_credit uuid:=public.atlas_account_id('IVA crédito fiscal'); v_cash uuid:=public.atlas_account_id('Caja y bancos'); v_ap uuid:=public.atlas_account_id('Cuentas por pagar');\nbegin\n perform public.atlas_guard_permission('purchases'); if not public.can_access_branch(p_branch_id) then raise exception 'Sucursal no autorizada'; end if; if p_qty<=0 or p_cost<0 then raise exception 'Cantidad o costo inválido'; end if;\n select coalesce(tax,0) into v_tax_rate from products where id=p_product_id and company_id=v_company; v_subtotal:=p_qty*p_cost; v_tax:=v_subtotal*v_tax_rate/100; v_total:=v_subtotal+v_tax;\n select 'C-'||lpad((count(*)+1)::text,5,'0') into v_number from purchases where company_id=v_company;\n insert into purchases(id,company_id,branch_id,supplier_id,document_number,total,payment_condition,status) values(v_purchase_id,v_company,p_branch_id,p_supplier_id,v_number,v_total,p_payment_condition,'Recibida');\n insert into purchase_items(purchase_id,product_id,quantity,unit_cost) values(v_purchase_id,p_product_id,p_qty,p_cost);\n insert into inventory(company_id,branch_id,product_id,stock,reserved) values(v_company,p_branch_id,p_product_id,p_qty,0) on conflict(branch_id,product_id) do update set stock=inventory.stock+excluded.stock; update products set cost=p_cost where id=p_product_id and company_id=v_company;\n insert into inventory_movements(company_id,branch_id,product_id,movement_type,quantity,note,created_by) values(v_company,p_branch_id,p_product_id,'COMPRA',p_qty,v_number,auth.uid());\n if p_payment_condition='cash' then if p_cash_account_id is null then raise exception 'Cuenta de caja requerida'; end if; update cash_accounts set balance=balance-v_total where id=p_cash_account_id and company_id=v_company; insert into cash_movements(company_id,cash_account_id,movement_type,amount,reference,note) values(v_company,p_cash_account_id,'OUT',v_total,v_number,'Compra de contado'); else insert into payables(company_id,supplier_id,reference,issue_date,due_date,total,balance,status) values(v_company,p_supplier_id,v_number,current_date,current_date+30,v_total,v_total,'Pendiente'); end if;\n perform public.atlas_post_journal(v_number,'Compra de mercancía',jsonb_build_array(\n  jsonb_build_object('account_id',v_inv,'debit',v_subtotal,'credit',0),\n  jsonb_build_object('account_id',v_vat_credit,'debit',v_tax,'credit',0),\n  jsonb_build_object('account_id',case when p_payment_condition='cash' then v_cash else v_ap end,'debit',0,'credit',v_total)));\n perform public.atlas_write_audit('CREATE','purchase',jsonb_build_object('purchase_id',v_purchase_id,'number',v_number,'total',v_total));\n return jsonb_build_object('id',v_purchase_id,'number',v_number,'subtotal',v_subtotal,'tax',v_tax,'total',v_total);\nend $$;\n\ncreate or replace function public.atlas_collect_receivable(p_receivable_id uuid,p_amount numeric,p_cash_account_id uuid) returns jsonb language plpgsql security definer set search_path=public as $$\ndeclare v_company uuid:=public.current_company_id(); v_r receivables%rowtype; v_apply numeric; v_cash uuid:=public.atlas_account_id('Caja y bancos'); v_ar uuid:=public.atlas_account_id('Cuentas por cobrar');\nbegin perform public.atlas_guard_permission('receivables'); select * into v_r from receivables where id=p_receivable_id and company_id=v_company for update; if v_r.id is null then raise exception 'Cuenta por cobrar no encontrada'; end if; v_apply:=least(p_amount,v_r.balance); if v_apply<=0 then raise exception 'Monto inválido'; end if; update receivables set balance=balance-v_apply,status=case when balance-v_apply<=0 then 'Pagada' else 'Parcial' end where id=v_r.id; update cash_accounts set balance=balance+v_apply where id=p_cash_account_id and company_id=v_company; insert into cash_movements(company_id,cash_account_id,movement_type,amount,reference,note) values(v_company,p_cash_account_id,'IN',v_apply,v_r.reference,'Cobro CxC'); perform public.atlas_post_journal(v_r.reference,'Cobro de cuenta por cobrar',jsonb_build_array(jsonb_build_object('account_id',v_cash,'debit',v_apply,'credit',0),jsonb_build_object('account_id',v_ar,'debit',0,'credit',v_apply))); return jsonb_build_object('applied',v_apply,'remaining',v_r.balance-v_apply); end $$;\n\ncreate or replace function public.atlas_pay_payable(p_payable_id uuid,p_amount numeric,p_cash_account_id uuid) returns jsonb language plpgsql security definer set search_path=public as $$\ndeclare v_company uuid:=public.current_company_id(); v_r payables%rowtype; v_apply numeric; v_cash uuid:=public.atlas_account_id('Caja y bancos'); v_ap uuid:=public.atlas_account_id('Cuentas por pagar');\nbegin perform public.atlas_guard_permission('payables'); select * into v_r from payables where id=p_payable_id and company_id=v_company for update; if v_r.id is null then raise exception 'Cuenta por pagar no encontrada'; end if; v_apply:=least(p_amount,v_r.balance); if v_apply<=0 then raise exception 'Monto inválido'; end if; update payables set balance=balance-v_apply,status=case when balance-v_apply<=0 then 'Pagada' else 'Parcial' end where id=v_r.id; update cash_accounts set balance=balance-v_apply where id=p_cash_account_id and company_id=v_company; insert into cash_movements(company_id,cash_account_id,movement_type,amount,reference,note) values(v_company,p_cash_account_id,'OUT',v_apply,v_r.reference,'Pago CxP'); perform public.atlas_post_journal(v_r.reference,'Pago de cuenta por pagar',jsonb_build_array(jsonb_build_object('account_id',v_ap,'debit',v_apply,'credit',0),jsonb_build_object('account_id',v_cash,'debit',0,'credit',v_apply))); return jsonb_build_object('applied',v_apply,'remaining',v_r.balance-v_apply); end $$;\n\ncreate or replace function public.atlas_create_expense(p_cash_account_id uuid,p_category text,p_description text,p_amount numeric,p_reference text) returns jsonb language plpgsql security definer set search_path=public as $$\ndeclare v_company uuid:=public.current_company_id(); v_id uuid:=gen_random_uuid(); v_ref text:=coalesce(nullif(p_reference,''),'G-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS')); v_cash uuid:=public.atlas_account_id('Caja y bancos'); v_exp uuid:=public.atlas_account_id('Compras / Gastos');\nbegin perform public.atlas_guard_permission('expenses'); if p_amount<=0 then raise exception 'Monto inválido'; end if; insert into expenses(id,company_id,cash_account_id,category,description,amount,reference,created_by) values(v_id,v_company,p_cash_account_id,p_category,p_description,p_amount,v_ref,auth.uid()); update cash_accounts set balance=balance-p_amount where id=p_cash_account_id and company_id=v_company; insert into cash_movements(company_id,cash_account_id,movement_type,amount,reference,note) values(v_company,p_cash_account_id,'OUT',p_amount,v_ref,p_description); perform public.atlas_post_journal(v_ref,'Gasto',jsonb_build_array(jsonb_build_object('account_id',v_exp,'debit',p_amount,'credit',0),jsonb_build_object('account_id',v_cash,'debit',0,'credit',p_amount))); return jsonb_build_object('id',v_id,'reference',v_ref,'amount',p_amount); end $$;\n\ngrant execute on function public.atlas_create_sale(uuid,uuid,text,uuid,jsonb) to authenticated;\ngrant execute on function public.atlas_create_purchase(uuid,uuid,text,uuid,uuid,numeric,numeric) to authenticated;\ngrant execute on function public.atlas_collect_receivable(uuid,numeric,uuid) to authenticated;\ngrant execute on function public.atlas_pay_payable(uuid,numeric,uuid) to authenticated;\ngrant execute on function public.atlas_create_expense(uuid,text,text,numeric,text) to authenticated;\n

-- ===== FASE 13: DEVOLUCIONES CONTABLES, SEGURIDAD Y DEPLOY =====

create or replace function public.atlas_require_operation_permission(p_operation text)
returns void
language plpgsql
security definer
set search_path=public
...
begin
  v_permission := case p_operation
    ...
    when 'collect_receivable' then 'receivables'
    when 'pay_payable' then 'payables'
    when 'transfer_stock' then 'stockTransfers'
    when 'expense' then 'expenses'
    when 'return_sale' then 'returns'
    when 'accounting' then 'accounting'
    else null end;

  if v_permission is null then raise exception 'Operación no reconocida'; end if;
  perform public.atlas_guard_permission(v_permission);
end $$;

grant execute on function public.atlas_require_operation_permission(text) to authenticated;
...
declare
 ...
 v_number text;
 v_sales_acc uuid:=public.atlas_account_id('Ventas');
 v_vat_acc uuid:=public.atlas_account_id('IVA por pagar');
 v_cash_acc uuid:=public.atlas_account_id('Caja y bancos');
 v_ar_acc uuid:=public.atlas_account_id('Cuentas por cobrar');
 v_inv_acc uuid:=public.atlas_account_id('Inventario');
 v_cogs_acc uuid:=public.atlas_account_id('Costo de ventas');
 v_credit_acc uuid;
begin
 perform public.atlas_require_operation_permission('return_sale');
 ...
 select * into v_line
 from sale_items
 where sale_id=v_sale.id and product_id=p_product_id;
 ...
 if v_returned+p_qty>v_line.quantity then
   raise exception 'Cantidad devuelta supera la vendida';
 end if;

 v_net := p_qty * v_line.unit_price;
 v_tax := v_net * coalesce(v_line.tax_rate,0) / 100;
 v_total := v_net + v_tax;

 select p_qty * coalesce(p.cost,0)
 into v_cost
 from products p
 where p.id=p_product_id and p.company_id=v_company;

 v_number := 'DV-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS');

 insert into sales_returns(company_id,sale_id,product_id,quantity,amount,status)
 values(v_company,v_sale.id,p_product_id,p_qty,v_total,'Completada');
 ...
 v_credit_acc := case
   when coalesce(v_sale.payment_condition,'cash')='credit' then v_ar_acc
   else v_cash_acc
 end;

 perform public.atlas_post_journal(
   v_number,
   'Devolución de venta',
   jsonb_build_array(
     jsonb_build_object('account_id',v_sales_acc,'debit',v_net,'credit',0),
     jsonb_build_object('account_id',v_vat_acc,'debit',v_tax,'credit',0),
     jsonb_build_object('account_id',v_credit_acc,'debit',0,'credit',v_total),
     jsonb_build_object('account_id',v_inv_acc,'debit',v_cost,'credit',0),
     jsonb_build_object('account_id',v_cogs_acc,'debit',0,'credit',v_cost)
   )
 );

 perform public.atlas_write_audit(
   'CREATE','return',
   jsonb_build_object('number',v_number,'sale_id',v_sale.id,'product_id',p_product_id,'qty',p_qty,'total',v_total,'cost',v_cost)
 );

 return jsonb_build_object('number',v_number,'qty',p_qty,'net',v_net,'tax',v_tax,'total',v_total,'cost',v_cost);
end $$;

revoke all on function public.atlas_return_sale(uuid,uuid,numeric) from public;
grant execute on function public.atlas_return_sale(uuid,uuid,numeric) to authenticated;

-- Contabilidad visible solo para usuarios con permiso.
drop policy if exists chart_accounts_company_all on chart_of_accounts;
create policy chart_accounts_company_all on chart_of_accounts
for all
using(company_id=public.current_company_id() and public.has_permission('accounting'))
with check(company_id=public.current_company_id() and public.has_permission('accounting'));
...
for select
using(
  public.has_permission('accounting')
  and exists(
...
revoke all on function public.atlas_financial_summary() from public;

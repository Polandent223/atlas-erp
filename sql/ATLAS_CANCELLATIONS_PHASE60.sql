-- ATLAS Fase 60 — Anulaciones atómicas de ventas y compras
...
declare src uuid; eid uuid:=gen_random_uuid();
begin
 if p_company is null or p_company<>public.current_company_id() then raise exception 'Empresa inválida'; end if;
 select id into src from public.journal_entries where company_id=p_company and reference=p_reference order by created_at asc limit 1;
 if src is null then raise exception 'Asiento original no encontrado para %',p_reference; end if;
...
begin
 ...
 -- Revierte todos los ingresos de caja vinculados a la venta/cobros de esa referencia.
 ...
 select * into ar from public.receivables where company_id=cid and sale_id=s.id order by balance desc limit 1 for update;
...
begin
 ...
 -- Debe existir stock suficiente para retirar exactamente lo comprado; si ya fue vendido, se bloquea.
 ...
 -- Revierte egresos de caja vinculados a la compra/pagos de esa referencia.
 ...
 select * into ap from public.payables where company_id=cid and purchase_id=p.id order by balance desc limit 1 for update;

...
export async function pullTransactions(localState){
 ...
 localState.customerCredits=customerCredits.map(r=>({id:r.id,customerId:r.customer_id,amount:Number(r.balance||0),balance:Number(r.balance||0),reference:r.reference||'',date:r.created_at,currency:r.currency||'USD',status:Number(r.balance||0)>0?'Disponible':'Utilizado'}));
 localState.supplierCredits=supplierCredits.map(r=>({id:r.id,supplierId:r.supplier_id,amount:Number(r.balance||0),balance:Number(r.balance||0),reference:r.reference||'',date:r.created_at,currency:r.currency||'USD',status:Number(r.balance||0)>0?'Disponible':'Utilizado'}));

...
function mapCustomer(r){return {id:r.id,code:"",name:r.name,taxId:r.tax_id||"",phone:r.phone||"",email:r.email||"",city:r.address||"",creditLimit:0,status:r.active===false?"Inactivo":"Activo",balance:0}}
function mapSupplier(r){return {id:r.id,code:"",name:r.name,taxId:r.tax_id||"",phone:r.phone||"",email:r.email||"",city:r.address||"",status:r.active===false?"Inactivo":"Activo",balance:0}}
function mapProduct(r){return {id:r.id,sku:r.sku||"",name:r.name,category:"General",cost:Number(r.cost||0),price:Number(r.price||0),tax:0,minStock:Number(r.min_stock||0),status:r.active===false?"Inactivo":"Activo"}}
...
export async function pullCoreWorkspace(localState){
 ...
 const [customers,suppliers,products,inventory,rates]=await Promise.all([RemoteRepo.list("customers",{order:{column:"created_at",ascending:true}}),RemoteRepo.list("suppliers",{order:{column:"created_at",ascending:true}}),RemoteRepo.list("products",{order:{column:"sku",ascending:true}}),RemoteRepo.list("inventory"),RemoteRepo.list("exchangeRates",{order:{column:"effective_at",ascending:false}})]);
 localState.customers=customers.map(mapCustomer); localState.suppliers=suppliers.map(mapSupplier); localState.products=products.map(mapProduct); localState.inventory=inventory.map(mapInventory); localState.exchangeRates=rates.map(mapRate);
 return {profile,counts:{customers:localState.customers.length,suppliers:localState.suppliers.length,products:localState.products.length,inventory:localState.inventory.length,exchangeRates:localState.exchangeRates.length}};
}
export async function pullTransactions(localState){
 ...
 localState.receivables=receivables.map(r=>({id:r.id,reference:r.reference,customerId:r.customer_id,saleId:r.sale_id,date:r.due_date||now(),dueDate:r.due_date||now(),total:Number(r.total||0),balance:Number(r.balance||0),status:r.status==="PAID"?"Pagada":"Pendiente"}));
 localState.payables=payables.map(r=>({id:r.id,reference:r.reference,supplierId:r.supplier_id,purchaseId:r.purchase_id,date:r.due_date||now(),dueDate:r.due_date||now(),total:Number(r.total||0),balance:Number(r.balance||0),status:r.status==="PAID"?"Pagada":"Pendiente"}));
 ...
 localState.sales=sales.map(r=>({id:r.id,number:r.number,date:r.created_at,customerId:r.customer_id,branchId:r.branch_id,subtotal:Number(r.subtotal||0),tax:Number(r.tax||0),total:Number(r.total||0),baseTotalUSD:Number(r.base_total_usd||r.total||0),currency:r.document_currency||"USD",documentTotal:Number(r.total||0),exchangeRate:Number(r.exchange_rate||1),condition:arBySale.has(r.id)?"credit":"cash",status:r.status==="ACTIVE"?"Completada":(r.status||"Completada"),items:saleLines[r.id]||[]}));
 localState.purchases=purchases.map(r=>({id:r.id,number:r.number,date:r.created_at,supplierId:r.supplier_id,branchId:r.branch_id,subtotal:Number(r.subtotal||0),tax:Number(r.tax||0),total:Number(r.total||0),baseTotalUSD:Number(r.base_total_usd||r.total||0),currency:r.document_currency||"USD",documentTotal:Number(r.total||0),exchangeRate:Number(r.exchange_rate||1),condition:apByPurchase.has(r.id)?"credit":"cash",status:r.status==="ACTIVE"?"Recibida":(r.status||"Recibida"),items:purchaseLines[r.id]||[]}));
 ...
 localState.expenses=expenses.map(r=>({id:r.id,date:r.created_at,category:"General",description:r.description||"",amount:Number(r.amount||0),currency:r.currency||"USD",accountId:r.cash_account_id,reference:r.reference||""}));

...
function printPurchase(id){
 ...
 const supplier=s.suppliers.find(x=>x.id===p.supplierId),currency=String(p.currency||"USD").toUpperCase(),rate=currency==="USD"?1:Number(p.exchangeRate||1);
 const rows=(p.items||[]).map(i=>{const prod=s.products.find(x=>x.id===i.productId),unit=Number(i.cost||i.price||0)*rate;return [esc(prod?.name||i.name||"Producto"),fmt(i.qty),moneyWithCurrency(unit,currency),moneyWithCurrency(Number(i.qty||0)*unit,currency)]});
 const voided=["anulada","cancelled","void"].includes(String(p.status||"").toLowerCase());
 docPrint(`Compra ${p.number||""}`,`${voided?'<div style="font-size:54px;font-weight:800;opacity:.15;text-align:center">ANULADA</div>':""}<h1>Compra ${esc(p.number||"")}</h1><p><b>Proveedor:</b> ${esc(supplier?.name||"")}</p><p><b>Fecha:</b> ${esc(p.date||"")}</p>${table(["Producto","Cant.","Costo","Total"],rows)}<h2>Total: ${moneyWithCurrency(p.documentTotal??Number(p.total||0)*rate,currency)}</h2>${currency!=="USD"?`<p><b>Base contable:</b> USD ${money(p.baseTotalUSD ?? p.total ?? 0)} · <b>Tasa:</b> ${money(rate)}</p>`:""}`);
}
...
function saleDocumentHTML(sale){
 ...
 const currency=String(sale.currency||"USD").toUpperCase(),rate=currency==="USD"?1:Number(sale.exchangeRate||1);
 const rows=(sale.items||[]).map(i=>{const p=s.products.find(x=>x.id===i.productId),unit=Number(i.price||0)*rate,line=unit*Number(i.qty||0);return `<tr><td>${esc(p?.name||i.productId)}</td><td class="num">${money(i.qty)}</td><td class="num">${moneyWithCurrency(unit,currency)}</td><td class="num">${moneyWithCurrency(line,currency)}</td></tr>`}).join("");
 return `${documentCompanyBlock()}<div class="doc-title"><div><h1>FACTURA / VENTA</h1><span class="badge">${esc(sale.status||"Completada")}</span></div><div class="doc-meta"><strong>${esc(sale.number)}</strong><br>${datefmt(sale.date)}<br>${esc(branch?.name||"")}</div></div><div class="doc-party"><strong>Cliente:</strong> ${esc(customer?.name||"Consumidor final")}${customer?.taxId?`<br><strong>${esc(fiscalConfig().taxIdLabel||"RIF")}:</strong> ${esc(customer.taxId)}`:""}${customer?.phone?`<br><strong>Tel.:</strong> ${esc(customer.phone)}`:""}</div><table><thead><tr><th>Producto</th><th class="num">Cant.</th><th class="num">Precio</th><th class="num">Total</th></tr></thead><tbody>${rows}</tbody></table><div class="totals"><div><span>Subtotal</span><strong>${moneyWithCurrency(sale.documentSubtotal??Number(sale.subtotal||0)*rate,currency)}</strong></div><div><span>Impuestos</span><strong>${moneyWithCurrency(sale.documentTax??Number(sale.tax||0)*rate,currency)}</strong></div><div class="grand"><span>Total</span><strong>${moneyWithCurrency(sale.documentTotal??Number(sale.total||0)*rate,currency)}</strong></div></div><div class="doc-note"><strong>Condición:</strong> ${(sale.condition||sale.paymentCondition)==="credit"?"Crédito":"Contado"}${currency!=="USD"?` · Base contable USD ${money(sale.baseTotalUSD ?? sale.total ?? 0)} · Tasa ${money(rate)}`:""}</div>${documentFooter()}`;
}
function quotationDocumentHTML(q){
 const s=state(),customer=s.customers.find(c=>c.id===q.customerId),currency=String(q.currency||"USD").toUpperCase(),rate=currency==="USD"?1:Number(q.exchangeRate||1);
 const rows=(q.items||[]).map(i=>{const p=s.products.find(x=>x.id===i.productId),unit=Number(i.price||0)*rate,line=unit*Number(i.qty||0);return `<tr><td>${esc(p?.name||i.productId)}</td><td class="num">${money(i.qty)}</td><td class="num">${moneyWithCurrency(unit,currency)}</td><td class="num">${moneyWithCurrency(line,currency)}</td></tr>`}).join("");
 return `${documentCompanyBlock()}<div class="doc-title"><div><h1>COTIZACIÓN</h1><span class="badge">${esc(q.status||"Vigente")}</span></div><div class="doc-meta"><strong>${esc(q.number)}</strong><br>${datefmt(q.date)}</div></div><div class="doc-party"><strong>Cliente:</strong> ${esc(customer?.name||"")}</div><table><thead><tr><th>Producto</th><th class="num">Cant.</th><th class="num">Precio</th><th class="num">Total</th></tr></thead><tbody>${rows}</tbody></table><div class="totals"><div><span>Subtotal</span><strong>${moneyWithCurrency(q.documentSubtotal??Number(q.subtotal||0)*rate,currency)}</strong></div><div><span>Impuestos</span><strong>${moneyWithCurrency(q.documentTax??Number(q.tax||0)*rate,currency)}</strong></div><div class="grand"><span>Total</span><strong>${moneyWithCurrency(q.documentTotal??Number(q.total||0)*rate,currency)}</strong></div></div><div class="doc-note">Esta cotización no representa una factura hasta ser convertida en venta.${currency!=="USD"?`<br><strong>Base referencial:</strong> USD ${money(q.total||0)} · <strong>Tasa:</strong> ${money(rate)}`:""}</div>${documentFooter()}`;
}
...
function returnDocumentHTML(r,isPurchase=false){
 const s=state(),source=isPurchase?s.purchases.find(v=>v.id===r.purchaseId):s.sales.find(v=>v.id===r.saleId),party=isPurchase?s.suppliers.find(x=>x.id===r.supplierId):s.customers.find(x=>x.id===source?.customerId),product=s.products.find(p=>p.id===r.productId);
 const currency=String(source?.currency||"USD").toUpperCase(),rate=currency==="USD"?1:Number(source?.exchangeRate||1),docAmount=Number(r.amount||0)*rate;
 return `${documentCompanyBlock()}<div class="doc-title"><div><h1>${isPurchase?"DEVOLUCIÓN DE COMPRA":"DEVOLUCIÓN DE VENTA"}</h1><span class="badge">${esc(r.status||"Completada")}</span></div><div class="doc-meta"><strong>${esc(r.number)}</strong><br>${datefmt(r.date)}</div></div><div class="doc-party"><strong>${isPurchase?"Proveedor":"Cliente"}:</strong> ${esc(party?.name||"")}<br><strong>Documento origen:</strong> ${esc(isPurchase?r.purchaseNumber:r.saleNumber)}</div><table><thead><tr><th>Producto</th><th class="num">Cantidad</th><th class="num">Monto devuelto</th></tr></thead><tbody><tr><td>${esc(product?.name||r.productId)}</td><td class="num">${money(r.qty)}</td><td class="num">${moneyWithCurrency(docAmount,currency)}</td></tr></tbody></table><div class="doc-note"><strong>Base contable:</strong> USD ${money(r.amount)}${currency!=="USD"?` · <strong>Tasa documento:</strong> ${money(rate)}`:""}${r.refundAmount?`<br><strong>Movimiento de caja:</strong> ${moneyWithCurrency(r.refundAmount,r.refundCurrency||"USD")}`:""}</div>${documentFooter()}`;
}
...
function newQuotation(){
 ...
 `,f=>{
   ...
   s.quotations.push({id:DB.id("q"),number,date,expiresAt:exp.toISOString(),customerId:f.get("customerId"),items:[{productId:p.id,qty,price:p.price,tax:p.tax}],subtotal,tax,total,baseTotalUSD:total,currency:"USD",documentSubtotal:subtotal,documentTax:tax,documentTotal:total,exchangeRate:1,status:"Pendiente"});
